...
def measure(target_fps: float, seconds: float, detector_fps: float, out_path: str) -> dict:
    record_source = MirrorFrameSource()
    record_source.probe()
    ...
    recorder = VideoRecorder(out_path, frame_source=record_source.grab_once, fps=target_fps)
...
def main() -> int:
    ...
    print(f"{'target':>7} {'achieved':>9} {'detector grab ms':>18} {'detector fps':>13}")
    for rate in args.rates:
        result = measure(rate, args.seconds, args.detector_fps, f"{args.out_prefix}_{rate:g}.mp4")
        print(
            f"{result['target_fps']:>7.0f} {result['achieved_fps']:>9.1f} "
            f"{result['detector_grab_median_ms']:>9.0f} (p90 {result['detector_grab_p90_ms']:>3.0f}) "
